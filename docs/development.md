...
| 规划 | 意图/执行策略分离、连续 `resource.read → plan.commit` 工具历史、直接 commit 与目录→详情→commit 的闭合状态机、一次安全 schema 工具错误后重提、读取/反馈/最终计划的持久轨迹顺序及 SSE/轮询/历史一致、紧凑工具 schema 与完整执行校验等价、稳定缓存前缀、context plan、完整目录排序/超限、过滤与图身份、越权键、额外模型调用数、三个根入口、空词面合法性、权重/通道矛盾、能力清单与硬预算 |
...
| 原文与回答 | 确定性来源准入、完整实际包/真实输入预算、PDF 原生结构元信息的闭合公共投影、ContextTree/稳定优先队列/集合分层、实际 path 与唯一 primary RQ 的 Mid 映射、多 Mid 去重/结构伴随/mandatory/全未映射直达、连续 `evidence.read → evidence.commit` 工具历史、一次与多次批量读取/空提交/安全错误修正/身份漂移/取消与恢复、未读 Mid 严格缩小、最终模型只收到 generation view 原文、部分回答与有界不足、一次最终原生 GFM 生成、合法引用即时转序号胶囊、非法引用原样输出、可见流与持久答案逐字一致、完整答案跨度只绑定实际生成工作集来源、无完成后正文替换、无生成后模型调用 |
| 来源绑定 | 包内 handle、原文/回答身份、范围与路径、事务提交/回滚、篡改和历史重放；旧来源失效不阻断历史阅读或新检索 |
| 直接回答 | 能力卡、零工具/引用、同会话复用、失败回检索、SSE/同步一致、新旧协议隔离 |
| 可观测性 | prepared/completed 区分、接纳后先保存用户问题、错误类型、证据目录/读取/冻结轨迹与状态轮询/历史顺序一致、每次 context plan 与 decision provider 往返和本地目录/read/freeze 计时、P0/P1 保留与工具对原子性、压缩/截断/恢复 replay 等价、append-only 可见流与最终回答逐字一致、引用列表在正文后独立提交、`first_response_ms`/`first_token_ms`、10 秒 SSE 传输保活、断线后按 run 恢复、失败/取消会话终态、敏感信息不泄露 |
...
| 配置 | 根 `.env`/`settings.json` 唯一键归属、分别原子更新、组合身份、文件变更但 Redis 未广播时的热加载与模型桥同步、生命周期、Redis 广播、单例刷新、重建/容器重建待办；设置页按“运行控制/检索入口”展示单次模型、整链、生成时限与 token 预算，初始化和保存 round-trip 不丢字段，不保留无执行语义的独立规划时限；生成时限 600 秒可保存、601 秒由前后端边界拒绝 |
...
冷构建统计从任务接纳到最终提交、缓存失效和 freshness 的总墙钟。QA 统计从接纳到最终回答/失败，记录意图/策略规划、来源定位、各通道提名与融合、分层图检索、装包、来源准入、证据目录、每次 decision、本地原文读取、工作集冻结、一次最终生成和绑定。共享准备与嵌套阶段不能重复相加。当前真实验收逐例核对每次模型调用和从接纳到终态的墙钟分别服从当轮 settings 快照中的单次与总时限。
...
`evidence/read` 或上下文工程行为改变时，使用两个资料库共至少十个独立问题，覆盖不同文档、问题族、语言和可用表示。逐例分列终态、确定性直达/实际 read、Mid/source 数、决策调用数、input/output/cache token、context plan 压缩/截断、provider 区间并集、TTFT、最终生成、总墙钟与非模型墙钟；非模型墙钟按请求区间减 provider 区间并集计算，要求保持秒级、可达步骤优先毫秒级。失败、超时、证据不足、部分与完整回答不得合并口径；压缩前后另用不共享验收题的公开样例核对任务等价与证据覆盖。
