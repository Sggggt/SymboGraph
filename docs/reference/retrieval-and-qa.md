...
LLM 在一次规划输出中分别返回 `intent` 与 `execution_strategy`。它只能使用原问题、用户明确的会话约束、非事实性会话摘要及服务器提供的有界 capability manifest。manifest 包含可用层、版本、索引状态、文档/语言数量、可用范围与预算上限，不包含旧答案、旧模型判决或未经核验的事实。

计划必须在检索前通过闭合 schema、本地权限和预算校验并持久化。模型不能调用工具、选择不存在的字段、修改图边、放宽用户范围或直接提交数据库 ID。执行器依据声明式字段行动。
