...
取消普通模式和摘要模式；用户不再通过界面模式决定粗层或中层入口。LLM 在正式检索前可选择直接规划，或顺序请求只读粗层资源后再规划。执行器校验每个动作及最终策略后行动。

### 规划前粗层资源读取

`coarse_resource_read_v1` 是规划阶段的只读观察，不是图检索、答案上下文或通用工具权限。首个模型动作可直接提交完整 Intent/ExecutionStrategy，也可请求当前资料库 active 粗节点的**全部标题目录**。目录按持久 `node_weight` 降序、稳定节点键破平，包含临时选择键、标题、有限元信息和权重。权重仅用于目录排序，不表示与问题的相关性。目录读取后，模型可以直接规划，或按临时键选择至多四个节点读取摘要、定义、范围与边界信息，然后必须规划。模型不能跳过目录直接索取详情，也不能重复读、跨资料库读或在检索结果出来后再读。无可用粗层时只允许直接规划。若完整计划首次违反闭合 schema，可用安全字段路径和版本化错误码让模型重提一次完整计划；执行器不自动修补词面，失败计划不进入正式检索。
...
规划模型的提示使用从闭合 Pydantic 契约确定性派生的紧凑 schema：保留字段、类型、枚举、必填、结构引用及校验边界，去掉给人阅读的标题、描述、默认值和示例注解；执行器仍以原始完整契约校验模型输出。系统说明只保留行动顺序、来源边界和合法权重等不可由字段结构表达的规则。提示大小按系统、任务、能力清单和观察分项计量；不能为了减少字符数丢失原问题、过滤范围、完整粗层目录或来源约束。大资料回答可包含较多已准入原文，压缩元信息时保持每个 source handle 的原文与标签无损可重建。

最多四次规划响应（含一次格式重提）与一次生成均受各自单次超时约束；整体 QA 硬时限还必须为非模型阶段预留时间。默认总时限为 540 秒，规划单次上限 60 秒、生成单次上限 240 秒；总时限不是额外模型调用许可，任一阶段超时仍单独报告技术失败。
...
在线 QA 的可归因阶段至少覆盖：接纳排队、会话准备、能力清单与 freshness、可选粗层 resource/read 的本地查询及各次模型往返、最终规划、来源定位、查询 embedding、每层 Dense/RQ/BM25 提名、融合、边读取、图遍历、结构恢复、Context Package、来源准入、一次生成、来源绑定和数据库提交。每个 span 保存单调时钟区间、parent、状态及有界计数；模型/网络、数据库 I/O 与 CPU 分开标识。
