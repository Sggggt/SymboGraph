...
class FineCluster(Base):
    __tablename__ = "fine_clusters"
    __table_args__ = (UniqueConstraint("graph_state_id", "cluster_key", name="uq_fine_cluster_state_key"),)
    ...
    node_type: Mapped[str] = mapped_column(String(64), default="fine_seed", index=True)
...
class FineClusterMembership(Base):
    __tablename__ = "fine_cluster_memberships"
    __table_args__ = (UniqueConstraint("fine_cluster_id", "chunk_id", name="uq_fine_cluster_chunk"),)

    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=generate_uuid)
    fine_cluster_id: Mapped[str] = mapped_column(ForeignKey("fine_clusters.id", ondelete="CASCADE"), index=True)
    chunk_id: Mapped[str] = mapped_column(ForeignKey("chunks.id", ondelete="CASCADE"), index=True)
...
class FineClusterEdge(Base):
    __tablename__ = "fine_cluster_edges"

    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=generate_uuid)
    graph_state_id: Mapped[str] = mapped_column(ForeignKey("chunk_relation_graph_states.id", ondelete="CASCADE"), index=True)
    source_cluster_id: Mapped[str] = mapped_column(ForeignKey("fine_clusters.id", ondelete="CASCADE"), index=True)
    target_cluster_id: Mapped[str] = mapped_column(ForeignKey("fine_clusters.id", ondelete="CASCADE"), index=True)
    edge_type: Mapped[str] = mapped_column(String(64), index=True)
    weight: Mapped[float] = mapped_column(Float, default=1.0)
    distance: Mapped[float] = mapped_column(Float, default=0.0, index=True)
    raw_strength: Mapped[float] = mapped_column(Float, default=1.0)
...
class MidConcept(Base):
    ...
    representative_chunk_ids_json: Mapped[list[str]] = mapped_column(JSON, default=list)
    support_fine_cluster_ids_json: Mapped[list[str]] = mapped_column(JSON, default=list)
    support_chunk_ids_json: Mapped[list[str]] = mapped_column(JSON, default=list)
...
class MidConceptMembership(Base):
    ...
    __table_args__ = (UniqueConstraint("mid_concept_id", "fine_cluster_id", name="uq_mid_concept_fine_cluster"),)
...
class MidConceptEdge(Base):
    ...
    support_fine_edge_ids_json: Mapped[list[str]] = mapped_column(JSON, default=list)
    support_fine_node_ids_json: Mapped[list[str]] = mapped_column(JSON, default=list)
...
class ContextGraphState(Base):
    ...
    fine_cluster_hash: Mapped[str] = mapped_column(String(64), index=True)
...
class BM25Record(Base):
    __tablename__ = "bm25_records"
    __table_args__ = (UniqueConstraint("chunk_id", "embedding_text_version", name="uq_bm25_chunk_text_version"),)

    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=generate_uuid)
    knowledge_base_id: Mapped[str] = mapped_column(ForeignKey("knowledge_bases.id", ondelete="CASCADE"), index=True)
    chunk_id: Mapped[str] = mapped_column(ForeignKey("chunks.id", ondelete="CASCADE"), index=True)
    embedding_text_version: Mapped[str] = mapped_column(String(64), index=True)
    tokenizer_version: Mapped[str] = mapped_column(String(64), default="bm25_tokenizer_v1", index=True)
    text_hash: Mapped[str] = mapped_column(String(64), index=True)
    token_count: Mapped[int] = mapped_column(Integer, default=0)
    term_frequencies_json: Mapped[dict] = mapped_column(JSON, default=dict)
...
class RetrievalTrace(Base):
    ...
    fine_cluster_hash: Mapped[str | None] = mapped_column(String(64), nullable=True, index=True)
...
class GraphRetrievalStep(Base):
    ...
    cycle_reward: Mapped[float] = mapped_column(Float, default=0.0)
    ambiguous_edge_decisions_json: Mapped[list[dict]] = mapped_column(JSON, default=list)
