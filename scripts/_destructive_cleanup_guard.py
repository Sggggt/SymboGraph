...
def build_legacy_cleanup_inventory(
    db,
    knowledge_base,
    *,
    legacy_profile_keys: set[str],
    ...
    knowledge_base_id = str(knowledge_base.id)
    impact = (
        "scrub legacy keys only from the selected knowledge base active "
        "profile; optionally clear legacy retrieval score payloads only for "
        "the selected knowledge base and normalize the exact shared env file"
    )
    builder = InventoryBuilder(
        operation="destroy_legacy_derived_data_legacy_scope",
        knowledge_base_id=knowledge_base_id,
        knowledge_base_name=str(knowledge_base.name),
        options={
            "clear_legacy_score_audit": bool(clear_legacy_score_audit),
            "normalize_env": bool(normalize_env),
            "active_profile_id": (
                str(knowledge_base.active_profile_id)
    ...
    if knowledge_base.active_profile_id:
        profile_query = select(StrategyProfile).where(
            StrategyProfile.id == knowledge_base.active_profile_id
        )
        profile_binding_query = select(KnowledgeBase).where(
            KnowledgeBase.active_profile_id
            == knowledge_base.active_profile_id
        )
    else:
        profile_query = select(StrategyProfile).where(false())
        profile_binding_query = select(KnowledgeBase).where(false())
    if lock_rows:
        profile_query = profile_query.with_for_update()
        profile_binding_query = profile_binding_query.with_for_update()
    profile_query = profile_query.order_by(StrategyProfile.id.asc())
    profile_binding_query = profile_binding_query.order_by(
    ...
    builder.add_scope(
        ...
        _ordered_scope(
            ...
            lambda row: (
                {
                    "id": str(row[0].id),
                    "profile_hash": str(row[0].profile_hash),
                    "profile_json_hash": hashed_value(
                        dict(row[0].profile_json or {})
                    ),
                    "legacy_keys": sorted(
                        set(row[0].profile_json or {})
                        & set(legacy_profile_keys)
                    ),
                }
                if set(row[0].profile_json or {}) & set(legacy_profile_keys)
                else None
            ),
        ),
    )
    builder.add_scope(
        ...
        _ordered_scope(
            db,
            profile_binding_query,
            lambda row: {
                "knowledge_base_id": str(row[0].id),
                "is_selected_target": str(row[0].id)
                == knowledge_base_id,
            },
        ),
    )
    ...
    step_query = (
        ...
        .join(
            RetrievalTrace,
            GraphRetrievalStep.retrieval_trace_id == RetrievalTrace.id,
        )
        .where(RetrievalTrace.knowledge_base_id == knowledge_base_id)
        .order_by(GraphRetrievalStep.id.asc())
    )
    if lock_rows:
        trace_query = trace_query.with_for_update()
        step_query = step_query.with_for_update(
            of=GraphRetrievalStep
        )
    builder.add_scope(
        "retrieval_trace_score_payloads",
        _ordered_scope(
            db,
            trace_query,
            lambda row: (
                {
                    "id": str(row[0].id),
                    "scores_hash": hashed_value(dict(row[0].scores_json)),
                }
                if clear_legacy_score_audit and row[0].scores_json
                else None
            ),
        ),
    )
    builder.add_scope(
        "graph_step_score_payloads",
        _ordered_scope(
            db,
            step_query,
            lambda row: (
                {
                    "id": str(row[0].id),
                    "retrieval_trace_id": str(row[0].retrieval_trace_id),
                    "score_hash": hashed_value(dict(row[0].score_json)),
                }
                if clear_legacy_score_audit and row[0].score_json
                else None
            ),
        ),
    )

    env_status = env_sync_status()
    env_targets: list[dict[str, Any]] = []
    if normalize_env:
        env_identity = runtime_env_file_identity(ENV_PATH)
        env_targets.append(
            {
                "path": env_identity["path"],
                "identity": env_identity,
                "identity_hash": env_identity["identity_hash"],
                "deprecated_keys": list(
                    env_status.get("deprecated_keys") or []
                ),
                "bom_keys": list(env_status.get("bom_keys") or []),
            }
        )
    builder.add_scope("shared_env_file", env_targets)
    return builder.finalize()
