...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run real layered retrieval queries and write score/trace diagnostics.")
...
async def main() -> None:
    ...
    with session_scope() as db:
            ...
            rows.append(
                {
                    ...
                    "top_scores": [item["score"] for item in result.results[:5]],
                    "rq": {
        ...
        payload = {
            ...
            "rows": rows,
            "pass": bool(rows) and all(row["result_count"] > 0 and row["trace_id"] and row["rq"]["query_rq_path"] and row["rq"]["candidate_count"] > 0 for row in rows),
        }
